...
+----------------+-----------------------+------+-----+------------+----------+
| pc_eid         | int(11) unsigned      | NO   | PRI | NULL       | auto_inc |
...
def _cleanup_textonly(s):
    # http://stackoverflow.com/questions/730299/replace-html-entities-with-the
    #corresponding-utf-8-characters-in-python-2-6
    s = s.strip()
    ...
    #    '&gt;': '>',
    #}
    ## http://stackoverflow.com/questions/2400504/easiest-way-to-replace-a-str
    #ing-using-a-dictionary-of-replacements/2401481#2401481
    ## no whole words
...
for event in eventrows:
    ...
    title = cleanTextFromControlChars(
        str(event["pc_title"]).decode(IN_ENCODING)
    )
